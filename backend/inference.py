...
def load_class_list(cfg: InferenceConfig) -> Tuple[str, ...]:
    """
    Stable class ordering for numeric IDs.
    """
...
def preprocess_audio_to_tensor(
    cfg: InferenceConfig, audio_path: str
    ...
    if x.shape[-2] != cfg.spec_size:
        x = torch.nn.functional.interpolate(
            x,
            size=(cfg.spec_size, cfg.spec_size),
            mode="bilinear",
            align_corners=False,
        )
...
# -------------------------------------------------------
def predict_topk(
    model: torch.nn.Module, x: torch.Tensor, top_k: int
...
def build_results(
    ...
    labels = [
        class_list[i] if i < len(class_list) else f"class_{i}"
...
# -------------------------------------------------------
# 🗂️ Temp File Utility (kept for compatibility)
# -------------------------------------------------------
